...
// 			}
			...
			else if (shape.opacity != 1 || i > (shapes.length - 1)) {
				shapes.push(clone_shape(shape));
				translate_shape(shapes[shapes.length - 1], 5, 5);
				return;
			}
...
function end_flag(flag_shape) {
	...
	shapes[shapes.length - 1].hyp_keep = true;
	
	connect_to_bottom(hyp_shape, flag_shape);
	for (i = 0; i < (shapes.length + hyp_shapes.length); i++) {
		if (i < shapes.length)
			shape = shapes[i];
...
function clear_hyp_keep() {
	var shape;
	for (i = 0; i < shapes.length; i++) {
		shape = shapes[i];
		shape.hyp_keep = false;
	}
}
